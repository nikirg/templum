...
| Setups | Named after their infrastructure | `RedisSetup`, `LocalSetup` |
...
- [ ] Add config class for new infrastructure to `config.py` if needed (nested `BaseModel`, `| None = None` if optional)
- [ ] Add the new name to the `Setup` enum in `config.py`
- [ ] Add a `case` for it in `Config.build_setup()` with a lazy import
- [ ] Add any required infrastructure packages via `uv add`
